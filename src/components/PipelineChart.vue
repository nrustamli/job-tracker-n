<template>
  <div class="bg-white rounded-xl border border-border p-5">
    <div class="flex items-center justify-between mb-4">
      <h2 class="font-semibold text-text-primary">Pipeline</h2>
      <span class="text-xs text-text-secondary uppercase tracking-wide">{{ total }} Total</span>
    </div>
    <div class="space-y-2.5">
      <div v-for="s in visibleStatuses" :key="s.key" class="flex items-center gap-3">
        ...
        <span class="text-sm text-text-primary w-24 shrink-0">{{ s.label }}</span>
    ...
    <div class="flex gap-4 mt-3 pt-3 border-t border-border">
      <div class="flex items-center gap-1.5">
        ...
        <span class="text-xs text-text-secondary">Rejected {{ counts.rejected }}</span>
      </div>
      <div class="flex items-center gap-1.5">
        <span class="w-2 h-2 rounded-full bg-status-ghosted"></span>
        <span class="text-xs text-text-secondary">Ghosted {{ counts.ghosted }}</span>
      </div>
    </div>
  </div>
</template>
